...
def build_augmented_dataset(majority_amount):
#majority_amount : maximum number of sunglasses examples in dataset
    compteur = 0
    image_set, labels = [], []
...
def build_model():
    ...
    path_model = globalize("/model_{}_layers_size_{}_{}/models").format(layers_number, shape, shape)
    path_logger = globalize("/model_{}_layers_size_{}_{}/csv_logger").format(layers_number, shape, shape)
    path_curves = globalize("/model_{}_layers_size_{}_{}/curves").format(layers_number, shape, shape)
    ...
    for batch in batches:
        for f in fs:
            for d in ds:
                model = architecture(dense_nb, [d], conv_nb, [f])
                ...
                # Callbacks
                model_file_path = path_model+"/model_f_{}_d_{}_batch_{}.h5".format(f, d, batch)
                checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath=model_file_path,
                ...
                history_conv = model.fit(images, classes, batch_size = batch, epochs = epochs,
                                         ...
                                         callbacks = [checkpoint, reduce_lr, csv_logger, earlystopping])
                print("Model trained")
                ...
                #save dataframe each time and we update it
                ...
                df.to_excel(globalize("/model_{}_layers_size_{}_{}/").format(layers_number, shape, shape) +
                            "resume_{}_layers.xlsx".format(layers_number))
