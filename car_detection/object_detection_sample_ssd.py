...
def main():
    ...
    model_name = os.path.split(os.path.dirname(args.model+'/*'))[1].replace('.xml', '')
    ...
    while True:
        ...
        for number, proposal in enumerate(res):
            if proposal[2] > 0:
                ...
                if confidence > args.confidence and label in [1,2]:
                    output.append((xmin, ymin, xmax, ymax, label,
                                   name[len(args.input+'/'):], confidence))
                    """if not args.save:
        ...
        for box in output:
            ...
            tab.to_csv(globalize(f"{args.tab_base}_{model_name}.csv"))
        ...
        if args.save:
            if has_video:
                ...
                cv2.imwrite(outdir + 'detection_' + model_name + '_' + base, img)
        else:
